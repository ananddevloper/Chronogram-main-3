...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  bool _syncLoading = true;
  bool _syncUpdating = false;
  ...
  // ✅ GET sync preference from API
  Future<void> _loadSyncPreference() async {
    final result = await ApiService.getSyncPreference();
    if (mounted) {
      setState(() {
        if (result["status"] == "success" && result["mode"] != null) {
          _syncMode = result["mode"];
        }
        _syncLoading = false;
      });
    }
  ...
  // ✅ PUT sync preference to API
  Future<void> _updateSyncPreference(String mode) async {
    if (_syncUpdating || _syncMode == mode) return;

    setState(() => _syncUpdating = true);

    final result = await ApiService.updateSyncPreference(mode);
      ...
      setState(() {
        if (result["status"] == "success") {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(result["message"] ?? "Failed to update sync preference"),
              backgroundColor: Colors.redAccent,
            ),
          );
        }
  ...
  // ✅ New dynamic sync tile (replaces old _buildSyncPreference)
  Widget _buildSyncTile({
    ...
    final bool isSelected = _syncMode == mode;

    return _SmoothClick(
      onTap: () => _updateSyncPreference(mode),
      child: Padding(
        padding: const EdgeInsets.all(15),
        child: Row(
          children: [
            Container(
              padding: const EdgeInsets.all(10),
              decoration: BoxDecoration(
                color: isSelected ? const Color(0xff3B260D) : Colors.white10,
                borderRadius: BorderRadius.circular(10),
              ),
              child: Icon(
                icon,
            ...
            const SizedBox(width: 10),
            // ✅ Spinner during update, radio icon otherwise
            _syncUpdating
                ? const SizedBox(
                    width: 20,
                    height: 20,
